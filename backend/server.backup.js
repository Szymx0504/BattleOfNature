...
io.on("connection", (socket) => {
  ...
  socket.on("playCard", (card, tile, special) => {
    ...
    if (cardProperties[card].type !== "spell") {
      ...
      if (
        (!["medicinal herbs", "bark beetles", "magic force"].includes(card) &&
        ...
        (card === "bark beetles" &&
          ...
          !checkOpponentsMainTree(row, col, rotate)) ||
        (["medicinal herbs", "magic force"].includes(card) && // "good" cards
          !game.board[row][col].cards.find(
      ...
      const targetCard =
        mainTree && !["medicinal herbs", "magic force"].includes(card)
          ? {
              name: "main tree",
      ...
      if (
        targetCard.name === "creepers" &&
        !["medicinal herbs", "magic force"].includes(card)
      ) {
      ...
      if (!["medicinal herbs", "magic force"].includes(card)) {
        ...
        // magic force for now
        if (targetCard.placedThisTurn) {
          socket.emit(
            "error",
            "The card has been placed this turn. It cannot be reactivated yet"
          );
          return;
        } else {
          // targetCard.hasAttack = true;
          // wrong here - should be added that performs an attack immediately. think about how to implement it
          // also - take care of cards with limited attacking abilities here like creepers, banana tree, cocoa tree etc
          // as well as potato etc etc
          // maybe extract more things into functions?
          // for now prowizorka
          // also add SPECIAL functionality on the frotnend
          if (!special) {
            socket.emit("error", "No target card specified");
            return;
          }
          let enemyCard;
          for (let i = 0; i < board.length; i++) {
            for (let j = 0; j < board[i].length; j++) {
              for (const card of board[i][j].cards) {
                if (card.name === special && card.owner === enemyId) {
                  enemyCard = card;
                  break;
                }
              }
          ...
          if (!enemyCard) {
            socket.emit("error", "No target card found");
            return;
          }
          changesVector.push({
            action: "reactivate",
          ...
          let dmgValue = special.dmg;
          if (special.name === "poplar") {
            dmgValue = 1;
            // will be extracted into a func, dont worry about it for now
          }
          dealDamage(enemyCard, dmgValue, changesVector);
        }
      }
    }
